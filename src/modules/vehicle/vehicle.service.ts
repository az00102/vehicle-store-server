...
import { UpdateVehiclePayload, Vehicle } from "../../interface/interface";
...
// Get Vehicle by ID
const getVehicleWithId = async (payload: number) => {
    // console.log("retriving...")
...
const deleteVehicle = async (payload: number) =>{
    ...
    const active_booking_check = await pool.query(
        `
        SELECT status
        FROM bookings
        WHERE vehicle_id = $1
    ...
    const result = await pool.query(
        `DELETE FROM Vehicles WHERE id = $1 RETURNING *`, [payload]
    )
