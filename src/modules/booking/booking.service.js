...
const createBooking = async (payload) => {
    ...
    //get vehicle 
    const vehicleResult = await db_1.pool.query(`
        SELECT vehicle_name, 
        daily_rent_price,
        availability_status
        FROM vehicles
    ...
    const rent_duration = Math.ceil((end.getTime() - start.getTime()) / milisec_per_day);
    if (rent_duration === 0) {
...
// Get All Bookings
const getAllBookings = async (current_user_id, current_user_role) => {
...
//update booking
const updateBooking = async (payload, current_user_role, booking_id, current_user_id) => {
    ...
    if (current_user_role === 'admin') {
        ...
        const result = await db_1.pool.query(`UPDATE bookings
            SET status = $1
            WHERE id = $2
                AND status = 'active'
